...
/**
 * spark streaming uniq_server 日志处理
 * 1、流量监控   5分钟 请求总量 请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 超时【5s】 50次
 */
public class SparkStreamingForUniqServerBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForUniqServerBatchAndMonitor.class);
    ...
    static {
        ...
        try {
            receiveMails[0] = new InternetAddress("futao.zhao@ifchange.com", "", "UTF-8");
            receiveMails[1] = new InternetAddress("zhi.liu@ifchange.com", "", "UTF-8");
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-uniq_server-monitor";
        ...
        jssc.checkpoint("/algorithm/uniq_server-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.filter(s -> {
            ...
            String value = s._2;
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("uniq_server", 1);
            else
                tuple2 = new Tuple2<>("uniq_server", 0);
            return tuple2;
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
                .reduceByKeyAndWindow((Function2<Integer, Integer, Integer>) (v1, v2) -> v1 + v2, Durations.minutes(5), Durations.minutes(5), 4);
        ...
        JavaPairDStream<String, Double> totalTime = lines.filter(s -> {
            ...
            return new Tuple2<>("uniq_server", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
                .reduceByKeyAndWindow((Function2<Double, Double, Double>) (v1, v2) -> v1 + v2, Durations.minutes(5), Durations.minutes(5), 4);

        JavaPairDStream<String, Tuple2<Double, Integer>> pair1 = totalTime.join(totalCount, 4);
        ...
        JavaPairDStream<String, Tuple2<Tuple2<Integer, Double>, Integer>> pair2 = totalFailCountByFAndW.join(totalTimeByFAndW, 4).join(totalNumByFAndW, 4);
        ...
        JavaPairDStream<String, Integer> pair3 = lines.filter(s -> {
            ...
            return tuple2;
                ...
                Durations.minutes(5), Durations.minutes(5), 4);
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `uniq_server_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
