...
/**
 * spark streaming position 日志处理
 * 1、流量监控   5分钟 请求总量、请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 失败 50次报警
 */
public class SparkStreamingForPositionBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForPositionBatchAndMonitor.class);
    ...
    static {
        ...
        try {
            receiveMails[0] = new InternetAddress("haifeng.wu@ifchange.com", "", "UTF-8");
            receiveMails[1] = new InternetAddress("hui.wang@ifchange.com", "", "UTF-8");
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-position-monitor";
        ...
        jssc.checkpoint("/basic_data/position-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.filter(s -> {
            ...
            String value = s._2;
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("position", 1);
            else
                tuple2 = new Tuple2<>("position", 0);
            return tuple2;
        ...
        JavaPairDStream<String, Double> totalTime = lines.filter(s -> {
            ...
            if (StringUtils.isNotBlank(data)) {
                try {
                    ...
                    logger.info("value解析失败,接收到的原始内容为:" + data);
                }
            }
            if (StringUtils.isNotBlank(value)) {
                try {
            ...
            return new Tuple2<>("position", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
        ...
        JavaPairDStream<String, Integer> pair3 = lines.filter(s -> {
            ...
            if (StringUtils.isNotBlank(value)) {
                ...
                if (null != map && map.size() > 0) {
                    String w = StringUtils.isNotBlank(map.get("w")) ? map.get("w") : "w";
                    String s = StringUtils.isNotBlank(map.get("s")) ? map.get("s") : "s";
                    String f = StringUtils.isNotBlank(map.get("f")) ? map.get("f") : "f";
                    String key = f + "->" + w;
                    if (s.equals("0")) {
                        tuple2 = new Tuple2<>(key, 1);
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `position_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
