...
/**
 * spark streaming tag_predict 日志处理
 * 1、流量监控   5分钟 请求总量、请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 失败50次
 */
public class SparkStreamingForTagPredictBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForTagPredictBatchAndMonitor.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-tag_predict-monitor";
        ...
        jssc.checkpoint("/algorithm/tag_predict-checkpoint");
        ...
        HashPartitioner partitioner = new HashPartitioner(5);
        ...
        JavaPairDStream<String, Integer> totalCount = lines.mapToPair((Tuple2<String, String> s) -> {
            ...
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("tag_predict", 1);
            else
                tuple2 = new Tuple2<>("tag_predict", 0);
            return tuple2;
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
                .reduceByKeyAndWindow((Function2<Integer, Integer, Integer>) (v1, v2) -> v1 + v2, Durations.minutes(5), Durations.minutes(5), 5);
        ...
        JavaPairDStream<String, Double> totalTime = lines.mapToPair(s -> {
            ...
            return new Tuple2<>("tag_predict", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
                .reduceByKeyAndWindow((Function2<Double, Double, Double>) (v1, v2) -> v1 + v2, Durations.minutes(5), Durations.minutes(5), 5);

        JavaPairDStream<String, Tuple2<Double, Integer>> pair1 = totalTime.join(totalCount, 5);
        ...
        JavaPairDStream<String, Tuple2<Tuple2<Integer, Double>, Integer>> pair2 = totalFailCountByFAndW.join(totalTimeByFAndW, 5).join(totalNumByFAndW, 5);
        ...
        JavaPairDStream<String, Integer> pair3 = lines.map(s -> s._2)
                ...
                .reduceByKeyAndWindow((Function2<Integer, Integer, Integer>) (v1, v2) -> v1 + v2,
                        Durations.minutes(5), Durations.minutes(5), 5);
