...
/**
 * spark streaming jd_feature 日志处理
 * 1、流量监控   5分钟 请求总量 请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 超时【1s】100次
 */
public class SparkStreamingForJdFeatureBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForJdFeatureBatchAndMonitor.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-jd_feature-monitor";
        SparkConf conf = new SparkConf();
        conf.setMaster(args[0]);
        conf.setAppName(appName);
        conf.set("app.logging.name", "spark-streaming-jd_feature-monitor");
        JavaSparkContext jsc = new JavaSparkContext(conf);
        JavaStreamingContext jssc = new JavaStreamingContext(jsc, Durations.minutes(1));
        jssc.checkpoint("/algorithm/jd_feature-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.filter(s -> {
            ...
            return new Tuple2<>("jd_feature", num);
        ...
        JavaPairDStream<String, Double> totalTime = lines.filter(s -> {
            ...
            if (StringUtils.isNotBlank(data)) {
                String value = null;
                try {
                    value = data.substring(data.indexOf("t="), data.length());
                    logger.info(value);
                } catch (Exception e) {
                    logger.info("substring data error," + e.getMessage());
                }
            ...
            return new Tuple2<>("jd_feature", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
        ...
        JavaPairDStream<String, Double> totalTimeByFAndW = lines.filter(s -> {
            ...
            return tuple2;
        }).combineByKey(s -> s, (x, y) -> x + y, (x, y) -> x + y, partitioner, true)
                .reduceByKeyAndWindow((Function2<Double, Double, Double>) (v1, v2) -> v1 + v2, Durations.minutes(5),
                        Durations.minutes(5), 2);
        ...
        JavaPairDStream<String, Integer> totalNumByFAndW = lines.filter(s -> {
            ...
            return tuple2;
                ...
                .reduceByKeyAndWindow((Function2<Integer, Integer, Integer>) (v1, v2) -> v1 + v2, Durations.minutes(5),
                        Durations.minutes(5), 2);
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `jd_feature_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
