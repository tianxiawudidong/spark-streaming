...
/**
 * spark streaming cv_title 日志处理
 * 1、流量监控   5分钟 请求总量 请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 超时【500ms】 50次
 */
public class SparkStreamingForCvTitleBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForCvTitleBatchAndMonitor.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-cv_title-monitor";
        ...
        jssc.checkpoint("/algorithm/cv_title-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.filter(s -> {
            ...
            String value = s._2;
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("cv_title", 1);
            else
                tuple2 = new Tuple2<>("cv_title", 0);
            return tuple2;
        ...
        JavaPairDStream<String, Double> totalTime = lines.filter(s -> {
            ...
            return new Tuple2<>("cv_title", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `cv_title_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
