...
/**
 * spark streaming jd_bi 日志处理
 * 1、流量监控   5分钟 请求总量 请求总时间
 * 2、性能监控   5分钟 f w 平均响应时间
 * 3、稳定性监控 5分钟 f w 失败率
 * 4、报警监控   5分钟 失败 100次
 */
public class SparkStreamingForJdBiBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForJdBiBatchAndMonitor.class);
    ...
    static {
        ...
        try {
            receiveMails[0] = new InternetAddress("yong.shen@ifchange.com", "", "UTF-8");
    ...
    public static void main(String[] args) throws Exception{
        ...
        String appName = "spark-streaming-jd_bi-monitor";
        ...
        jssc.checkpoint("/algorithm/jd_bi-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.mapToPair((Tuple2<String, String> s) -> {
            ...
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("jd_bi", 1);
            else
                tuple2 = new Tuple2<>("jd_bi", 0);
            return tuple2;
        ...
        JavaPairDStream<String, Double> totalTime = lines.mapToPair(s -> {
            ...
            if (StringUtils.isNotBlank(data)) {
                try {
                    val = data.substring(data.indexOf("f="), data.length());
                } catch (Exception e) {
                    logger.info(data + ",substring error," + e.getMessage());
                }
            }
            if (StringUtils.isNotBlank(val)) {
                try {
                    map = ParamParseUtil.parse(val);
                } catch (Exception e) {
                    logger.info(val + " 转换成map报错," + e.getMessage());
                }
            }
            ...
            return new Tuple2<>("jd_bi", responseTime);
        ...
        //4、报警监控
        JavaPairDStream<String, Integer> pair3 = lines.mapToPair(str -> {
            String data = str._2();
            Tuple2<String, Integer> tuple2;
            Map<String, String> map = null;
            String val = "";
            ...
            if (null != map && map.size() > 0) {
                String w = StringUtils.isNotBlank(map.get("w")) ? map.get("w") : "w";
                String s = StringUtils.isNotBlank(map.get("s")) ? map.get("s") : "0";
                if (s.equals("0")) {
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `jd_bi_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
