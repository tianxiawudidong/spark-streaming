...
/**
 * spark streaming edps 日志处理
 * 1、统计5分钟的量 和花费的时间
 * 2、批量监控 5分钟超时【响应时间大于10s】50次的
 * 5分钟内，超过10s，50次
 * <p>
 * update at 18/1/3
 * 统计5分钟内，服务标记、请求work、平均响应时间
 */
public class SparkStreamingForEdpsBatchAndMonitor {

    private static final Logger logger = Logger.getLogger(SparkStreamingForEdpsBatchAndMonitor.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        String appName = "spark-streaming-edps-monitor";
        ...
        jssc.checkpoint("/algorithm/edps-checks");
        ...
        JavaPairDStream<String, Integer> totalCount = lines.filter(s -> {
            ...
            String value = s._2;
            if (StringUtils.isNotBlank(value))
                tuple2 = new Tuple2<>("edps", 1);
            else
                tuple2 = new Tuple2<>("edps", 0);
            return tuple2;
        ...
        JavaPairDStream<String, Double> totalTime = lines.filter(s -> {
            ...
            return new Tuple2<>("edps", responseTime);
        }).combineByKey(s -> s, (value1, value2) -> value1 + value2, (value1, value2) -> value1 + value2, partitioner, true)
        ...
        pair1.foreachRDD(s -> {
            ...
            if (null != data) {
                ...
                String sql = "insert into `edps_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
