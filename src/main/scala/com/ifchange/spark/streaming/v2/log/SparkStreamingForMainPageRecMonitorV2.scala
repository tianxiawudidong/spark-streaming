...
/**
  ...
  * 5、汇总统计   5分钟汇总一次
  * redis统计
  * 失败、超时信息保存hbase、mysql
  * Created by Administrator on 2018/1/11.
  * update at 5/25
  * 使用mysql 替换 mysql pool 减少连接数
  */
class SparkStreamingForMainPageRecMonitorV2 {

}

object SparkStreamingForMainPageRecMonitorV2 {

  private val logger = Logger.getLogger(classOf[SparkStreamingForMainPageRecMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val MAIN_PAGE_REC_SUCCESS = "mainPageRec_success"

  private val MAIN_PAGE_REC_FAIL = "mainPageRec_fail"

  private val hbasePool = new HbasePool("mainPageRec_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-mainpage_recommendation-monitor"
    ...
    ssc.checkpoint("/algorithm/mainpage_recommendation-checkpoint")
    ...
    val totalCount = lines.filter(rdd => {
      ...
      if (StringUtils.isNotBlank(kafkaValue))
        ("mainpage_recommendation", 1)
      else
        ("mainpage_recommendation", 0)
    ...
    val totalTime = lines.filter(rdd => {
      ...
      if (map.nonEmpty) {
        ...
        ("mainpage_recommendation", responseTime)
      } else
        ("mainpage_recommendation", 0.0)
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = MAIN_PAGE_REC_SUCCESS + "-" + day
            val failKey = MAIN_PAGE_REC_FAIL + "-" + day
          ...
          if (s.equals("0") || responseTime >= 1000) {
            ...
            hbaseClient.setTbale("mainPageRec_log")
            try {
            ...
            val sql = "insert into `mainpage_recommendation_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `mainpage_recommendation_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
    ...
    pair.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        val from = dtf.format(before)
        val to = dtf.format(localDateTime)
        val typeName = "mainpage_recommendation"
      ...
      if (count > 0) {
        ...
        for (data <- list) {
          ...
          if (num >= 100) {
            ...
            //发送邮件
            val subject = "mainpage_recommendation日志报警"
