...
/**
  ...
  */
class SparkStreamingForUserActMonitorV2 {

}

object SparkStreamingForUserActMonitorV2 {

  private val logger = Logger.getLogger(classOf[SparkStreamingForUserActMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val USER_ACT_SUCCESS = "userAct_success"

  private val USER_ACT_FAIL = "userAct_fail"

  private val hbasePool = new HbasePool("userAct_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-user_act-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-user_act-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/algorithm/user_act-checkpoint")
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = USER_ACT_SUCCESS + "-" + day
            val failKey = USER_ACT_FAIL + "-" + day
          ...
          if (s == "0" || responseTime >= 1000) {
            logger.info("------------------")
            logger.info(data)
            logger.info("------------------")
            import collection.JavaConversions._
            val hbaseClient = hbasePool.getHbaseClient
            hbaseClient.setTbale("userAct_log")
            try {
            ...
            val sql = "insert into `user_act_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `user_act_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
