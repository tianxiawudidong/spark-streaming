...
/**
  ...
  */
class SparkStreamingForUniqServerMonitorV2 {

}

object SparkStreamingForUniqServerMonitorV2 {

  private val logger = Logger.getLogger(classOf[SparkStreamingForUniqServerMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val UNIQ_SUCCESS = "uniq_success"

  private val UNIQ_FAIL = "uniq_fail"

  private val hbasePool = new HbasePool("uniq_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-uniq_server-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-uniq_server-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(5))
    ssc.checkpoint("/algorithm/uniq_server-checkpoint")
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (s.equals("0") || responseTime >= 5000) {
            ...
            if (responseTime >= 5000) reason = "调用w:" + w + ",c:" + c + ",m:" + m + ",响应时间:" + r + ",超过5s"
            val sql = "insert into `uniq_server_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `uniq_server_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
