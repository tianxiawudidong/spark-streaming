...
/**
  ...
  * 5、汇总统计   5分钟汇总一次
  * redis统计
  * 超时、失败信息保存hbase、mysql
  * update 8/21 remove hbase
  * Created by Administrator on 2018/1/11.
  */
class SparkStreamingForJdFeatureMonitor {

}

object SparkStreamingForJdFeatureMonitor {

  private val logger = Logger.getLogger(classOf[SparkStreamingForJdFeatureMonitor])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val JD_FEATURE_SUCCESS = "jdFeature_success"

  private val JD_FEATURE_FAIL = "jdFeature_fail"

//  private val hbasePool = new HbasePool("jdFeature_log", "hadoop105,hadoop107,hadoop108", "2181")
...
//      new InternetAddress("dongjun.xu@ifchange.com", "", "UTF-8"))
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-jd_feature-monitor"
    ...
    ssc.checkpoint("/algorithm/jd_feature-checkpoint")
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = JD_FEATURE_SUCCESS + "-" + day
            val failKey = JD_FEATURE_FAIL + "-" + day
...
//            hbaseClient.setTbale("jdFeature_log")
//            try {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `jd_feature_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
