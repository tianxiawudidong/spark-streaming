...
/**
  ...
  * 5、汇总统计   5分钟汇总一次
  * redis统计
  * 错误、超时信息保存hbase、mysql
  * update 8/21 remove hbase
  * Created by Administrator on 2018/1/11.
  */
class SparkStreamingForCvEntityMonitor {

}

object SparkStreamingForCvEntityMonitor {

  private val logger = Logger.getLogger(classOf[SparkStreamingForCvEntityMonitor])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val CV_ENTITY_SUCCESS = "cvEntity_success"

  private val CV_ENTITY_FAIL = "cvEntity_fail"

  //  private val hbasePool = new HbasePool("cvEntity_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-cv_entity-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-cv_entity-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/algorithm/cv_entity-checkpoint")
          ...
          try {
            ...
            if (map.nonEmpty) {
              ...
              if (StringUtils.isNotBlank(time)) {
                ...
                val successKey = CV_ENTITY_SUCCESS + "-" + day
                val failKey = CV_ENTITY_FAIL + "-" + day
              ...
              if (s.equals("0") || responseTime >= 1000) {
                ...
                val sql = "insert into `cv_entity_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
                logger.info(sql)
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `cv_entity_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
