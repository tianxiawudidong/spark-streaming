...
/**
  ...
  */
class SparkStreamingForIcdcMonitor {

}

object SparkStreamingForIcdcMonitor {

  private val logger = Logger.getLogger(classOf[SparkStreamingForIcdcMonitor])
      ...
      rdds.foreach(rdd => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            hostname match {
              case "jcsj-web38.echeng" => redisCli.incr(redis, host38Key)
              case "jcsj-web39.echeng" => redisCli.incr(redis, host39Key)
              case "BI-70.echeng" => redisCli.incr(redis, host70Key)
              case "search28.echeng" => redisCli.incr(redis, search28Key)
              case "search29.echeng" => redisCli.incr(redis, search29Key)
