...
/**
  ...
  */
class SparkStreamingForTagPredictEdpsMonitorV2 {

}

object SparkStreamingForTagPredictEdpsMonitorV2 {

  private val logger = Logger.getLogger("SparkStreamingForTagPredictEdpsMonitorV2")

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val TAG_PREDICT_FOR_EDPS_SUCCESS = "tagPredictForEdps_success"

  private val TAG_PREDICT_FOR_EDPS_FAIL = "tagPredictForEdps_fail"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-tag_predict_for_edps-monitor"
    ...
    ssc.checkpoint("/algorithm/tag_predict_for_edps-checkpoint")
    ...
    val totalCount = lines.map(rdd => {
      ...
      if (StringUtils.isNotBlank(kafkaValue))
        ("tag_predict_for_edps", 1)
      else
        ("tag_predict_for_edps", 0)
    ...
    val totalTime = lines.map(rdd => {
      ...
      if (map.nonEmpty) {
        ...
        ("tag_predict_for_edps", responseTime)
      } else
        ("tag_predict_for_edps", 0.0)
      ...
      rdds.foreach(value => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = TAG_PREDICT_FOR_EDPS_SUCCESS + "-" + day
            val failKey = TAG_PREDICT_FOR_EDPS_FAIL + "-" + day
            ...
            val host54Key = "tag_predict_for_edps-54-" + day
            val host56Key = "tag_predict_for_edps-56-" + day
            hostName match {
              case "192.168.8.54" =>
          ...
          if (s.equals("0")) {
            ...
            val sql = "insert into `tag_predict_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 1 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        val from = dtf.format(before)
        val to = dtf.format(localDateTime)
        val typeName = "tag_predict_for_edps"
      ...
      if (count > 0) {
        ...
        for (data <- list) {
          ...
          if (num >= 50) {
            ...
            //发送邮件
            val subject = "tag_predict_for_edps日志报警"
