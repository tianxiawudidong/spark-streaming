...
/**
  ...
  */
class SparkStreamingForCvWorkYearMonitorV2 {

}

object SparkStreamingForCvWorkYearMonitorV2{
  private val logger = Logger.getLogger(classOf[SparkStreamingForCvWorkYearMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val CV_WORKYEAR_SUCCESS = "cvWorkYear_success"

  private val CV_WORKYEAR_FAIL = "cvWorkYear_fail"

  private val hbasePool = new HbasePool("cvWorkYear_log", "hadoop105,hadoop107,hadoop108", "2181")
...
//  private val mysqlPool = new MysqlPools(MysqlConfig.USERNAME, MysqlConfig.PASSWORD, MysqlConfig.HOST, MysqlConfig.PORT, MysqlConfig.DBNAME)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-cv_workyear-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-cv_workyear-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(5))
    ssc.checkpoint("/algorithm/cv_workyear-checkpoint")
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = CV_WORKYEAR_SUCCESS + "-" + day
            val failKey = CV_WORKYEAR_FAIL + "-" + day
          ...
          if (s.equals("0") || responseTime >= 500) {
            ...
            hbaseClient.setTbale("cvWorkYear_log")
            ...
            logger.info(reason)
            val sql = "insert into `cv_workyear_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `cv_workyear_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
