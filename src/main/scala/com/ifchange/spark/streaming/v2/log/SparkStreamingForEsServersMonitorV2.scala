...
/**
  ...
  */
class SparkStreamingForEsServersMonitorV2 {

}

object SparkStreamingForEsServersMonitorV2 {

  private val logger = Logger.getLogger(classOf[SparkStreamingForEsServersMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val ES_SERVERS_SUCCESS = "esServers_success"

  private val ES_SERVERS_FAIL = "esServers_fail"

  private val hbasePool = new HbasePool("esServers_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-es_servers-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-es_servers-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/algorithm/es_servers-checkpoint")
    ...
    val summaryValue = lines.mapPartitions(rdds => {
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          val functionMark = "esServers_" + logId
          val errNo = map.getOrElse("err_no", "未知")
          val errMsg = map.getOrElse("err_msg", "未知")
          var reason = ""
          if (StringUtils.isNotBlank(time)) {
            val day = time.split(" ")(0)
            val successKey = ES_SERVERS_SUCCESS + "-" + day
            val failKey = ES_SERVERS_FAIL + "-" + day
          ...
          if (s.equals("0") || responseTime >= 1000) {
            ...
            hbaseClient.setTbale("esServers_log")
            ...
            if (responseTime >= 1000) reason = "调用w:" + w + ",c:" + c + ",m:" + m + ",响应时间:" + r + ",超过1s"

            val sql = "insert into `es_servers_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `es_servers_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
