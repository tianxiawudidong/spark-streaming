...
/**
  * GSYSTEM
  * 1、流量监控   5分钟总的请求量、总的请求时间
  * 2、性能监控   5分钟 每个f、worker的平均响应时间
  * 3、稳定性监控 5分钟 f w 失败率
  * 4、报警监控   5分钟 超时50次
  * 5、汇总统计   5分钟汇总一次
  * redis统计
  ...
  */
class SparkStreamingForGsystemMonitorV2 {

}

object SparkStreamingForGsystemMonitorV2 {

  private val logger = Logger.getLogger(classOf[SparkStreamingForGsystemMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val GSYSTEM_SUCCESS = "gsystem_success"

  private val GSYSTEM_FAIL = "gsystem_fail"

  private val hbasePool = new HbasePool("gsystem_log", "hadoop105,hadoop107,hadoop108", "2181")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-gsystem-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-gsystem-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/basic_data/gsystem-checks")
    ...
    val pair1 = totalCount.join(totalTime, 3)
    ...
    val totalTimeByFAndW = lines.map(rdd => {
      ...
      if (map.nonEmpty) {
        val f = map.getOrElse("f", "f").split("_")(0)
    ...
    val pair = totalFailCountByFAndW.join(totalTimeByFAndW, 3).join(totalCountByFAndW, 3)
    ...
    val summaryValue = lines.mapPartitions(rdds => {
      ...
      rdds.foreach(value => {
        ...
        if (map.nonEmpty) {
          ...
          //hostname
          val hostname: String = map.getOrElse("hostname", "hostname未知")
          //r 频响时间
          val r: String = map.getOrElse("r", "0")
          val responseTime: Int = r.toInt
          //w work名
          ...
          var rowKey = ""
          if (StringUtils.isNotBlank(f) && f.contains("_"))
            try {
              rowKey = f.split("_")(1)
            } catch {
              case e: Exception => logger.info("get logid error," + e.getMessage)
            }
          val functionMark = "gsystem_" + rowKey
          //w work名
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey: String = GSYSTEM_SUCCESS + "-" + day
            val failKey: String = GSYSTEM_FAIL + "-" + day
            ...
            // 增加统计具体主机上的请求次数
            val host13Key: String = "gsystem-jcsjweb13-" + day
            val host39Key: String = "gsystem-jcsjweb39-" + day
            val host70Key: String = "gsystem-bi70-" + day
            //gsystem总数
            ...
            if (!redis.exists(host13Key)) redisCli.set(redis, host13Key, "0")
            if (!redis.exists(host70Key)) redisCli.set(redis, host70Key, "0")
            if (hostname.contains("pv13")) redisCli.incr(redis, host13Key)
          ...
          if (s.equals("0") || responseTime >= 5000) {
            ...
            hbaseClient.setTbale("gsystem_log")

            logger.info("--------------")
            logger.info(kafkaValue)
            logger.info("--------------")
            if (s.equals("0")) reason = "调用w:" + w + "报错，errNo:" + errNo + ",errMsg:" + errMsg
            if (responseTime >= 5000) reason = "调用w:" + w + ",频响时间超过5秒"
            import collection.JavaConversions._
            try {
              if (StringUtils.isNotBlank(rowKey)) {
                logger.info("save " + rowKey + " into hbase")
            ...
            val sql = "insert into log_monitor(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 2 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
