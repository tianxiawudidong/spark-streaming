...
/**
  ...
  */
class SparkStreamingForCropTagMonitor {

}

object SparkStreamingForCropTagMonitor {

  private val logger = Logger.getLogger(classOf[SparkStreamingForCropTagMonitor])
    ...
    val monitorValue = lines.filter(s => {
      ...
      if (map.nonEmpty && map.size > 1) {
        ...
        if (w.equals("null"))
          w = "w"
        val key = f + "-->" + w
        if (responseTime >= 1000 || s.equals("0"))
          (key, 1)
        else
          (key, 0)
      } else
