...
/**
  * cv_title
  * 1、流量监控   5分钟请求总量 请求总时间
  * 2、性能监控   5分钟 f w 平均响应时间
  * 3、稳定性监控 5分钟 f w 失败率
  * 4、报警监控   5分钟超时【100ms】 50次
  * 5、汇总统计   5分钟汇总一次
  * redis汇总
  * 错误、超时保存hbase、mysql
  ...
  */
class SparkStreamingForCvTitleMonitor {

}

object SparkStreamingForCvTitleMonitor {
  ...
  private val CVTITLE_SUCCESS = "cv_title_success"

  private val CVTITLE_FAIL = "cv_title_fail"

//  private val hbasePool = new HbasePool("cvTitle_log", "hadoop105,hadoop107,hadoop108", "2181")
...
//      new InternetAddress("dongjun.xu@ifchange.com", "", "UTF-8"))
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-cv_title-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-cv_title-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/algorithm/cv_title-checkpoint")
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty && map.size > 1) {
          ...
          if (StringUtils.isNotBlank(time)) {
            ...
            val successKey = CVTITLE_SUCCESS + "-" + day
            val failKey = CVTITLE_FAIL + "-" + day
          ...
          if (s.equals("0") || responseTime >= 100) {
            logger.info("----------------")
            logger.info(data)
            logger.info("----------------")
//            val hbaseClient = hbasePool.getHbaseClient
//            hbaseClient.setTbale("cvTitle_log")
...
//            }
            ...
            if (responseTime >= 100) reason = "调用w:" + w + ",c:" + c + ",m:" + m + ",响应时间：" + r + ",超过100ms"
            val sql = "insert into `cv_title_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `cv_title_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
