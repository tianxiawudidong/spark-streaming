...
/**
  ...
  */
class SparkStreamingForEdpsMonitorV2 {

}

object SparkStreamingForEdpsMonitorV2{
  private val logger = Logger.getLogger(classOf[SparkStreamingForEdpsMonitorV2])

  private val dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  private var redisCli = new RedisCli("192.168.8.117", 6070)

  private val EDPS_SUCCESS = "edps_success"

  private val EDPS_FAIL = "edps_fail"

  private val hbasePool = new HbasePool("edps_log", "hadoop105,hadoop107,hadoop108", "2181")
...
//  private val mysqlPool = new MysqlPools(MysqlConfig.USERNAME, MysqlConfig.PASSWORD, MysqlConfig.HOST, MysqlConfig.PORT, MysqlConfig.DBNAME)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topic = args(1)
    val groupId = args(2)
    val appName = "spark-streaming-edps-monitor"
    val conf = new SparkConf()
    conf.setAppName(appName)
    conf.setMaster(master)
    conf.set("app.logging.name", "spark-streaming-edps-monitor")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Durations.minutes(1))
    ssc.checkpoint("/algorithm/edps-checkpoint")
    ...
    val totalCount = lines.filter(s => {
      ...
      .reduceByKeyAndWindow((x: Int, y: Int) => x + y, Durations.minutes(5), Durations.minutes(5), 4)
    ...
    val totalTime = lines.filter(s => {
      ...
      .reduceByKeyAndWindow((x: Double, y: Double) => x + y, Durations.minutes(5), Durations.minutes(5), 4)

    val pair1 = totalCount.join(totalTime, 4)
    ...
    val pair = totalFailCountByFAndW.join(totalTimeByFAndW, 4).join(totalCountByFAndW, 4)
    ...
    val summaryValue = lines.mapPartitions(rdds => {
      ...
      rdds.foreach(f = value => {
        ...
        if (map.nonEmpty) {
          ...
          if (s == "0" || responseTime >= 10000) {
            logger.info("-------------")
            logger.info(data)
            logger.info("-------------")
            val hbaseClient = hbasePool.getHbaseClient
            hbaseClient.setTbale("edps_log")
            ...
            var reason = ""
            if (s == "0") reason = "调用w:" + w + ",c:" + c + ",m:" + m + ",调用失败" + "，errNo:" + errNo + ",errMsg:" + errMsg
            if (responseTime >= 10000) reason = "调用w:" + w + ",c:" + c + ",m:" + m + ",响应时间:" + r + ",超过10s"
            val sql = "insert into `edps_log_monitor`(`time`,`function_mark`,`is_success`,`response_time`,`work_name`,`alarm_reason`,`type`,`f`,`c`,`m`) " + "values(\"" + time + "\",\"" + functionMark + "\"," + s + "," + r + ",\"" + w + "\"," + "\"" + reason + "\"," + 0 + ",\"" + f + "\",\"" + c + "\",\"" + m + "\"" + ")"
            logger.info(sql)
            try {
              mysql.execute(sql)
            } catch {
    ...
    pair1.foreachRDD(rdd => {
      ...
      if (count > 0) {
        ...
        for (array <- list) {
          ...
          val sql = "insert into `edps_log_batch_result`(`from_time`,`end_time`,`total_num`,`total_use_time`,`type`)" +
